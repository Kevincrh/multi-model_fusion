...
class pc_view_dp_data(Dataset):
    def __init__(self, pc_root, view_root, depth_root, base_model_name=models.ALEXNET, status='train',
                 pc_input_num=1024):
        super(pc_view_dp_data, self).__init__()
        ...
        if status == STATUS_TRAIN:
            jpg_list_view = glob(osp.join(view_root, '*', 'train', '*.jpg'))
            jpg_list_depth = glob(osp.join(depth_root, '*', 'train', '*.jpg'))
            npy_list = glob(osp.join(pc_root, '*', 'train', '*.npy'))
        else:
            jpg_list_view = glob(osp.join(view_root, '*', 'test', '*.jpg'))
            jpg_list_depth = glob(osp.join(depth_root, '*', 'test', '*.jpg'))
            npy_list = glob(osp.join(pc_root, '*', 'test', '*.npy'))
        pc_dict = get_info(npy_list, isView=False)
        view_dict = get_info(jpg_list_view, isView=True)
        dp_dict = get_info(jpg_list_depth, isView=True)

        for name in pc_dict.keys():
            self.view_list.append(view_dict[name])
            self.dp_list.append(dp_dict[name])
    ...
    def __getitem__(self, idx):
        ...
        views = [self.transform(Image.open(v)) for v in self.view_list[idx]]
        depth = [self.transform(Image.open(v).convert('RGB')) for v in self.dp_list[idx]]
        lbl = self.lbl_list[idx]
        pc = np.load(self.pc_list[idx])[:self.pc_input_num].astype(np.float32)
        pc = normal_pc(pc)
        # if self.status == STATUS_TRAIN:
        ...
        return torch.stack(views).float(), torch.stack(depth).float(), torch.from_numpy(pc).float(), lbl
...
if __name__ == '__main__':
    ...
    pvd = pc_view_dp_data(status=STATUS_TEST)
    batch_len = len(pvd)
