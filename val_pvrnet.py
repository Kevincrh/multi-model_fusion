...
def main():
    ...
    val_loader = DataLoader(val_dataset, batch_size=config.pvd_net.train.batch_sz,
    ...
    optimizer_all = optim.SGD(net.parameters(), config.pvd_net.train.all_lr,
                              momentum=config.pvd_net.train.momentum,
                              weight_decay=config.pvd_net.train.weight_decay)

    print(f'loading pretrained model from {config.pvd_net.ckpt_load_file}')
    checkpoint = torch.load(config.pvd_net.ckpt_load_file)
